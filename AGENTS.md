...
The root of the repo contains an `index.html` file which describes and links to the various webtools. This file is auto-generated by the `generate-index.js` script, which scans the `/web/` directory, extracts the title and description from each tool's `index.html`, and creates links to all tools. A pre-commit git hook ensures the index is regenerated before each commit.

## Additional guidance

- DO NOT use React!
- If JavaScript dependencies are absolutely necessary, load them from a CDN
- Each webtool's `index.html` must have a meaningful `<title>` tag
- Each webtool's `index.html` must have a `<meta name="description">` tag with a short, single sentence (no period)
