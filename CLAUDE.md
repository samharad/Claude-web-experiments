...
The root of the repo contains an `index.html` file which describes and links to the various webtools. This file is auto-generated by the `generate-index.sh` script, which scans the `/web/` directory and creates links to all tools. A pre-commit git hook ensures the index is regenerated before each commit.
